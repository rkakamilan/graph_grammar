...
Train_params = {
    ...
    "model_params": {  # Parameter for the model
        ...
        "prod_rule_embed": [
            ...
            0
        ],  # Embedding method of a production rule. The simple embedding was the best, but each production rule could be embedded using GNN
        "prod_rule_embed_params": {
        ...
        "criterion_func_kwargs": {"beta": 0.01},
    },  # Parameters for the loss function. `beta` specifies beta-VAE.
    ...
    "seed_list": [
        ...
        1004,
        50243,
    ],  # seeds used for restarting training
...
TrainWithPred_params["model_params"]["predictor_list"] = [
    "Linear"
]  # Configuration of layers of predictors
TrainWithPred_params["model_params"]["predictor_out_dim_list"] = [
    1
]  # each layer's output dimension

Sample_params = {"seed": 123}
...
Encode_params = {"batch_size": 128}

ConstructDatasetForBO_params = {"batch_size": 128, "seed": 123}
...
MultipleBayesianOptimization_params = {
    "seed_list": [
        ...
        4213,
        12387460,
    ]  # the length of this list determines the number of Bayesian optimization executions
}
...
ConstrainedMolOpt_params = {
    "inseq_opt_params": {"lr": 1e-1, "num_iter": 80},
    "seed": 123,
}
